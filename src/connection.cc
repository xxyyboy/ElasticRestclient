 ...
 *
 */
RestClient::Connection::Connection(const std::string& baseUrl, const std::string username, const std::string password)
 ...
 *
 */
void RestClient::Connection::SetFileProgressCallback(curl_progress_callback progressFn) {
 ...
 *
 */
void RestClient::Connection::SetBasicAuth(const std::string& username,const std::string& password) {
...
RestClient::Response RestClient::Connection::performCurlRequest(const std::string& uri) {
    ...
    if (this->basicAuth.username.length() > 0) {
        std::string authString = std::string(this->basicAuth.username + ":" + this->basicAuth.password);
    ...
    /** set error buffer */
    curl_easy_setopt(this->curlHandle, CURLOPT_ERRORBUFFER, this->curlErrorBuf);
    ...
    if (this->followRedirects == true) {
        ...
        curl_easy_setopt(this->curlHandle, CURLOPT_MAXREDIRS, static_cast<int64_t>(this->maxRedirects));
    }
    ...
    if (!this->certType.empty()) {
        curl_easy_setopt(this->curlHandle, CURLOPT_SSLCERTTYPE, this->certType.c_str());
    }
    // set key file path
    ...
    if (!this->keyPassword.empty()) {
        curl_easy_setopt(this->curlHandle, CURLOPT_KEYPASSWD, this->keyPassword.c_str());
    }
    ...
    if (!this->unixSocketPath.empty()) {
        curl_easy_setopt(this->curlHandle, CURLOPT_UNIX_SOCKET_PATH, this->unixSocketPath.c_str());
    }
    ...
    curl_easy_getinfo(this->curlHandle, CURLINFO_TOTAL_TIME, &this->lastRequest.totalTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_NAMELOOKUP_TIME, &this->lastRequest.nameLookupTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_CONNECT_TIME, &this->lastRequest.connectTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_APPCONNECT_TIME, &this->lastRequest.appConnectTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_PRETRANSFER_TIME, &this->lastRequest.preTransferTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_STARTTRANSFER_TIME, &this->lastRequest.startTransferTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_REDIRECT_TIME, &this->lastRequest.redirectTime);
    curl_easy_getinfo(this->curlHandle, CURLINFO_REDIRECT_COUNT, &this->lastRequest.redirectCount);
    // free header list
 ...
 * @return response struct
 */
RestClient::Response RestClient::Connection::post(const std::string& url, const std::string& data) {
...
RestClient::Response RestClient::Connection::put(const std::string& url, const std::string& data) {
    ...
    curl_easy_setopt(this->curlHandle, CURLOPT_READFUNCTION, RestClient::Helpers::read_callback);
    /** set data object to pass to callback function */
    curl_easy_setopt(this->curlHandle, CURLOPT_READDATA, &up_obj);
    /** set data size */
    curl_easy_setopt(this->curlHandle, CURLOPT_INFILESIZE, static_cast<int64_t>(up_obj.length));
 ...
 * @return response struct
 */
RestClient::Response RestClient::Connection::patch(const std::string& url, const std::string& data) {
    /** initialize upload object */
    RestClient::Helpers::UploadObject up_obj;
    up_obj.data = data.c_str();
    up_obj.length = data.size();
 ...
 * @return response struct
 */
RestClient::Response RestClient::Connection::del(const std::string& url, const std::string& data) {
    /** we want HTTP DELETE */
